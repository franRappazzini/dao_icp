...
actor class BOX(init : { initial_mints : [{ account : { owner : Principal; subaccount : ?Blob }; amount : Nat }]; minting_account : { owner : Principal; subaccount : ?Blob }; token_name : Text; token_symbol : Text; decimals : Nat8; transfer_fee : Nat }) = this {
    ...
    func classifyTransfer(log : TxLog, transfer : Transfer) : Result<(Operation, Tokens), TransferError> {
      ...
      if from == minter -> send normally
      if to == self -> burn tokens (before: if to == minter -> burn tokens)
      else transfer
    ...
    */
      ...
      (#Burn(transfer), 0);
        ...
        let result = (#Transfer(transfer), effectiveFee);
        // };
        #Ok(result);
    };
    ...
    public shared ({ caller }) func icrc2_approve({
        ...
        Debug.print(debug_show (balance(approverAccount, log), overflowOk(approverBalance - effectiveFee)));

        // TODO ver porque el assert falla. preguntar en community
